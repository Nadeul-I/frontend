...
export default {
  ...
  watch: {
      ...
      if (this.getPlanNo != 0) {
        ...
        this.mapData.map((item) => {
          ...
          let infoContent = `<div class="info">
                                ...
                                <div class="info_content">
                                    ...
                                    <div class="info_detail">
                                        <div class="info_address1">${
                                          item.addr1
                                        }</div>
                                        <div class="info_address2">${
                                          item.addr2 == ""
                                            ? ""
                                            : "(" + item.addr2 + ")"
                                        }</div>
          ...
          if (this.searchLink && this.mapData.length == 1) {
            (info.toggle = true), info.setMap(this.map);
          }
    ...
    startPoint: function () {
      this.startPoint.keyword != null
        ? (this.start = true)
        : (this.start = false);
      if (this.startPoint.keyword != null && this.destPoint.keyword != null)
        this.find();
  ...
  methods: {
    ...
    find() {
      ...
      let startPosition = new kakao.maps.LatLng(
        this.startPoint.latitude,
        this.startPoint.longitude
      );
      let destPosition = new kakao.maps.LatLng(
      ...
      let halfPosition = new kakao.maps.LatLng(
        (this.startPoint.latitude + this.destPoint.latitude) / 2,
        (this.startPoint.longitude + this.destPoint.longitude) / 2
      );
    ...
    getTimeHTML(distance) {
      ...
      if (walkkTime >= 2400) {
        ...
        numberSpan.appendChild(
          document.createTextNode(`${day}일 ${hour}시간 `)
        );
      } else if (walkkTime > 60) {
        // walkHour = '<span class="number">' + Math.floor(walkkTime / 60) + '</span>시간 '
        numberSpan.appendChild(
          document.createTextNode(`${Math.floor(walkkTime / 60)}시간 `)
        );
      }
      ...
      if (bycicleTime > 60) {
        numberSpan3.appendChild(
          document.createTextNode(`${Math.floor(bycicleTime / 60)}시간 `)
        );
      }
      let numberSpan4 = document.createElement("span");
      numberSpan4.setAttribute("class", "number");

      numberSpan4.appendChild(
        document.createTextNode(`${bycicleTime % 60}분 `)
      );
      let dist =
      ...
      saveBtn.setAttribute("class", "planBtn");
      saveBtn.onclick = () => {
      ...
      modifyBtn.setAttribute("class", "planBtn");
      modifyBtn.onclick = () => {
      ...
      deleteBtn.setAttribute("class", "planBtn");
      deleteBtn.onclick = () => {
        this.deleteInfo(this.getPlanNo);
      };
      if (this.getPlanNo == 0) {
        saveInfo.appendChild(saveBtn);
      } else {
    ...
    saveInfo(value) {
      ...
      let newPlan = {
        ...
        planImg:
          this.destPoint.firstImage == ""
            ? this.startPoint.firstImage
            : this.destPoint.firstImage,
